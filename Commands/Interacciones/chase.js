...
module.exports = {
    ...
    aliases: ["perseguir"],
    usage: "chase <@Usuario>",
    description: "Persigue al usuario mencionado.",
    cooldown: 5,
    /**
    ...
    async execute(message, args, interaction, commandName, client, Discord) {
        try {
            ...
            if (user.id == message.author.id) {
                return message.channel.send({ content: `${emojis.negativo} | No puedes perseguirte a tí mismo.` })
            }
            const { soyultro } = require('soyultro')
            let author = message.author.username
            const embed = new MessageEmbed()
                .setTitle(
                    `${author} persiguió a ${user.user.username}`
                )
                .setColor("RANDOM")
                .setImage(soyultro('chase'))
                .setFooter("Impulsado por Zafiro™")
