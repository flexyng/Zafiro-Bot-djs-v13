    ...
    */
    async execute(message, args, commandName, client, Discord) {
        ...
        if (findPerms?.allowed.includes(commandName) && !findPerms?.denied.includes(commandName)) {
            ...
            if (!User) {
                return message.reply({ content: `${emojis.negativo} | Debes de mencionar a un usuario`, allowedMentions: { repliedUser: false } })
            }
            const Warns = await Warn.find({ guildId: message.guild.id, warnedId: User.user.id });
            console.log(Warns)
            if (!Warns) {
                return message.reply({ content: `${emojis.positivo} | Este usuario no tiene warns en este servidor`, allowedMentions: { repliedUser: false } });
            }
            if (Warns.length <= 0) {
                return message.reply({ content: `${emojis.positivo} | Este usuario no tiene warns en este servidor`, allowedMentions: { repliedUser: false } });
            }
            const AwaitResponse = new MessageEmbed()
                ...
                .setColor("BLUE")
            const msg = await message.reply({ embeds: [AwaitResponse], allowedMentions: { repliedUser: false } });
            ...
            if (!message.guild.me.permissions.has("KICK_MEMBERS")) {
                return message.reply({ content: `${emoji.negativo} | No puedo interactuar con este comando sin los siguientes permisos: \`KICK_MEMBERS\``, allowedMentions: { repliedUser: false } });
            }
            const User = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
            if (!User) {
                return message.reply({ content: `${emojis.negativo} | Debes de mencionar a un usuario`, allowedMentions: { repliedUser: false } })
            ...
            const AwaitResponse = new MessageEmbed()
                .setTitle("Cargando...")
                .setDescription(`${emojis.cargando} | \`Obteniendo registros...\``)
                .setColor("BLUE")
            const msg = await message.reply({ embeds: [AwaitResponse], allowedMentions: { repliedUser: false } });
            const embedDescription = Warns.map(
                (warn, i) => `\n**${i + 1}** - Razon: \`${warn.razon}\` - Creado: \`${warn.warnCreated}\` - ID: \`${warn.warnId}\` - Moderador: ${message.guild.members.cache.get(warn.modId)}`
