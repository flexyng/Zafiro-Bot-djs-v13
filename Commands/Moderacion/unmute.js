    ...
    async execute(message, args, commandName, client, Discord, prefix) {
        ...
        if (findPerms?.allowed.includes(commandName) && !findPerms?.denied.includes(commandName)) {
            if (!message.guild.me.permissions.has("MANAGE_ROLES")) {
                return message.reply({ content: `${emojis.negativo} | No puedo interactuar con este comando sin los siguientes permisos: \`MANAGE_ROLES\``, allowedMentions: { repliedUser: false } });
            }
            ...
            if (MuteRole) {
                ...
                Target.roles.remove(Role).catch((e) => {
                    return message.channel.send({ content: `${emojis.negativo} | Algo salio mal, porfavor contacta con mi soporte: https://discord.gg/SUM6mc7Ye3 ` });
                });
            ...
            if (MuteRole) {
                ...
                if (Razon.length > 250) {
                    return message.reply({ content: `${emojis.negativo} | La razón no puede ser mayor a 250 caracteres`, allowedMentions: { repliedUser: false } });
                }
                if (!Target.roles.cache.has(Role.id)) {
                    return message.reply({ content: `${emojis.negativo} | Este usuario no esta muteado`, allowedMentions: { repliedUser: false } });
                }
                Target.roles.remove(Role).catch((e) => {
                    return message.channel.send({ content: "Algo salio mal, porfavor contacta con mi soporte: https://discord.gg/SUM6mc7Ye3" });
                });
                const desmuteUser = await Muted.findOneAndDelete({ userId: Target.user.id, guildId: message.guild.id })
                const Embed = new MessageEmbed()
                    .setDescription(`**${targetName} - (${Target.user.id})** fue desmuteado\n\n**Moderador:** ${author} - ${message.author.id}\n**Razón:** ${Razon}\n**Día:** ${moment().format("Do/MMMM/YYYY").replace("º", "")}`)
                    .setFooter("Impulsado por Zafiro™")
                    .setTimestamp()
                    .setColor("BLUE")
                const Embed2 = new MessageEmbed()
                    .setAuthor(message.guild.name, message.guild.iconURL({ dynamic: true }) || client.user.avatarURL({ dynamic: true }))
                    .setDescription(`Has sido desmuteado del servidor **${message.guild.name}**\n\n**Moderador:** ${author} - ${message.author.id}\n**Razón:** ${Razon}\n**Día:** ${moment().format("Do/MMMM/YYYY").replace("º", "")}`)
                    .setFooter("Impulsado por Zafiro™")
                    .setTimestamp()
                    .setColor("BLUE")
                message.reply({ embeds: [Embed], allowedMentions: { repliedUser: false } });
