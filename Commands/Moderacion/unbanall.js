...
module.exports = {
    name: "unbanall",
    aliases: ["uball"],
    description: "Desbanea a todos los miembros baneados de este servidor",
    ...
    async execute(message, args, commandName, client, Discord) {
        ...
        if (findPerms?.allowed.includes(commandName) && !findPerms?.denied.includes(commandName)) {
            ...
            await message.guild.bans.fetch().then(bans => {
                if (bans.size === 0) {
                    return message.reply({ content: `${emojis.positivo} | \`No existen baneos en este servidor\` `, allowedMentions: { repliedUser: false } });
                }
                ...
                for (const ban of bans) {
                    message.guild.members.unban(ban.user.id, {
                        reason: `UnbanAll By ${message.author.username}`
                    });
                }
            ...
            await message.guild.bans.fetch().then(bans => {
                ...
                const AwaitBans = new MessageEmbed()
                    .setTitle("Información")
                    .setAuthor(message.guild.name, message.guild.iconURL({ dynamic: true }))
                    .setDescription(`${emojis.cargando} | \`Desbaneando a ${bans.size} usuario(s) en este servidor...\` `)
                    .setColor("BLUE")
                ...
                const ReadyBans = new MessageEmbed()
                    ...
                    .setTitle("Información")
                    .setAuthor(message.guild.name, message.guild.iconURL({ dynamic: true }))
                    .setDescription(`${emojis.positivo} | \`Termine de desbanear a todos correctamente\` `)
                    .setColor("GREEN")
                message.channel.send({ embeds: [ReadyBans] });
            })
        } else {
