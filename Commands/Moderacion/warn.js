    ...
    */
    async execute(message, args, commandName, client, Discord, prefix) {
        ...
        if(findPerms?.allowed.includes(commandName) && !findPerms?.denied.includes(commandName)) {
            ...
            if(!Usuario) {
                return message.reply({ content: `${emojis.negativo} | Tienes que proporcionar a un usuario para warnearlo`, allowedMentions: { repliedUser: false } });
            }
            ...
            let channelId;
            if(Razon?.length >= 250) {
                return message.reply({ content: `${emojis.negativo} | La razon no puede ser mayor a 250 caracteres`, allowedMentions: { repliedUser: false } });
            }
            if(!FindAnteriores) {
                Anteriores = 0;
            } else {
            ...
            channel.send({ embeds: [
                {
                    title: "Nuevo warn",
                    description: `**${targetName} - (${Target.id})** fue warneado.`,
                    fields: [
                        {
                            name: "Moderador",
