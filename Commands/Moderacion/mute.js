    ...
    async execute(message, args, commandName, client, Discord, prefix) {
        ...
        if (findPerms?.allowed.includes(commandName) && !findPerms?.denied.includes(commandName)) {
            if (!message.guild.me.permissions.has("MANAGE_ROLES")) {
                return message.reply({ content: `${emojis.negativo} | No puedo interactuar con este servidor sin los siguientes permisos: \`MANAGE_ROLES\``, allowedMentions: { repliedUser: false } });
            }
            ...
            if (MuteRole) {
                ...
                return message.reply({ content: `${emojis.negativo} | No existe un rol predeterminado para mutear, establecelo con \`${prefix}set-mute-role\``, allowedMentions: { repliedUser: false } });
            }
            ...
            if (MuteRole) {
                ...
                await createNewMuted.save()
                const Embed = new MessageEmbed()
                    .setDescription(`**${targetName} - (${Target.user.id})** fue muteado\n\n**Moderador:** ${author} - (${message.author.id})\n**Razón:** ${Razon}\n**Día:** ${moment().format("Do/MMMM/YYYY").replace("º", "")}`)
                    .setTimestamp()
                    .setColor("BLUE")
                const Embed2 = new MessageEmbed()
                    .setAuthor(message.guild.name, message.guild.iconURL({ dynamic: true }) || client.user.avatarURL({ dynamic: true }))
                    .setDescription(`Fuiste muteado en el servidor **${message.guild.name}**\n\n**Moderador:** ${author} - (${message.author.id})\n**Razón:** ${Razon}\n**Día:** ${moment().format("Do/MMMM/YYYY").replace("º", "")}`)
