    ...
    async execute(message, args, commandName, client, Discord, prefix) {
        ...
        if(findPerms?.allowed.includes(commandName) && !findPerms?.denied.includes(commandName)) {
            ...
            client.users.fetch(Target).then(user => {
                ...
                message.channel.send({ content: `${emojis.negativo} | Debes de especificar a un usuario.` })
            });
            ...
            client.users.fetch(Target).then(user => {
                message.channel.send({ content: `${emojis.positivo} | \`${user.username}\` fue baneado de este servidor.` });
            }).catch(e => {
                message.channel.send({ content: `${emojis.negativo} | Ese usuario no existe.` })
            });
        } else {
