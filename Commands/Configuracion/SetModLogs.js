    ...
    async execute(message, args, commandName, client, Discord, prefix) {
        if(!message.member.permissions.has("MANAGE_GUILD")) {
            ...
            if(!modLogs) {
                ...
                .setDescription(`${emojis.cargando} | \`¿Estas seguro de activar los Logs de Moderación en este servidor?\``)
                .setColor("BLUE")
                .setTimestamp()
                ...
                if(modLogs.actived) {
                    ...
                    const Embed = new MessageEmbed()
                    .setTitle("Información")
                    .setDescription(`${emojis.cargando} | \`¿Estas seguro de desactivar los Logs de Moderación en este servidor?\``)
                    .setColor("BLUE")
                    .setTimestamp()
                    let confirmacion = await message.reply({ embeds: [Embed], components: [row], allowedMentions: { repliedUser: false } });
                    const filter = i => i.user.id === message.author.id;
