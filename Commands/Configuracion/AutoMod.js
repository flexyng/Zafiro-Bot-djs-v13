...
module.exports = {
    ...
    async execute(message, args, commandName, client, Discord, prefix) {
        if(!message.member.permissions.has("ADMINISTRATOR")) {
            ...
            if(!mutedrole) {
                return message.reply({ content: `${emojis.negativo} | No has establecido un rol para mutear, colocalo con \`${prefix}setmuterole\``, allowedMentions: { repliedUser: false } });
            }
            if(antilinks) {
            ...
            if(!automod) {
                ...
                .setDescription(`${emojis.cargando} | \`¿Estas seguro de activar el Auto-Moderador en este servidor?\``)
                .setColor("BLUE")
                .setTimestamp()
                ...
                if(automod.actived) {
                    ...
                    const Embed = new MessageEmbed()
                    .setTitle("Información")
                    .setDescription(`${emojis.cargando} | \`¿Estas seguro de desactivar el Auto-Moderador en este servidor?\``)
                    .setColor("BLUE")
                    .setTimestamp()
                    let confirmacion = await message.reply({ embeds: [Embed], components: [row], allowedMentions: { repliedUser: false } });
                    const filter = i => i.user.id === message.author.id;
