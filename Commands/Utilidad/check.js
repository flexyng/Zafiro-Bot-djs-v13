...
module.exports = {
    ...
    async execute(message, args, commandName, client, Discord, prefix) {
        if(!message.member.permissions.has("ADMINISTRATOR")) {
            return message.channel.send({ content: `${emojis.negativo} | No tienes __permisos de administrador__ para ejecutar esto.`});
        }
        
        const embed = new MessageEmbed()
        .setColor("GREEN")
        .setTitle("Información")
        .setDescription(`${emojis.positivo} - Termine de comprobar mis permisos sin ningún error.`)
        .setFooter("¡Comprobador de permisos! | Impulsado por Zafiro™ Bot")
