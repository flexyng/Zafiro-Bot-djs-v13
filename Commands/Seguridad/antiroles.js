    ...
    async execute(message, args, commandName, client, Discord) {
        if(message.guild.ownerId !== message.author.id) {
            ...
            if(!antiroles) {
                ...
                .setDescription(`${emojis.cargando} | \`¿Estas seguro de activar el Anti-Roles en este servidor?\``)
                .setColor("BLUE")
                .setTimestamp()
                ...
                if(antiroles?.actived) {
                    ...
                    const Embed = new MessageEmbed()
                    .setTitle("Información")
                    .setDescription(`${emojis.cargando} | \`¿Estas seguro de desactivar el Anti-Roles en este servidor?\``)
                    .setColor("BLUE")
                    .setTimestamp()
                    let confirmacion = await message.reply({ embeds: [Embed], components: [row], allowedMentions: { repliedUser: false } });
                    const filter = i => i.user.id === message.author.id;
