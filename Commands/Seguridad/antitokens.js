    ...
    async execute(message, args, commandName, client, Discord, prefix) {
        if(message.guild.ownerId !== message.author.id) {
            ...
            if(!antitokens) {
                ...
                .setDescription(`${emojis.cargando} | \`¿Estas seguro de activar el Anti-Tokens en este servidor?\``)
                .setColor("BLUE")
                .setTimestamp()
                ...
                if(antitokens?.actived) {
                    ...
                    .setDescription(`${emojis.cargando} | \`¿Estas seguro de desactivar el Anti-Tokens en este servidor?\``)
                    .setColor("BLUE")
                    .setTimestamp()
                    let confirmacion = await message.reply({ embeds: [Embed], components: [row], allowedMentions: { repliedUser: false } });
                    const filter = i => i.user.id === message.author.id;
