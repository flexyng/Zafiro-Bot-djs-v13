...
module.exports = {
    ...
    async execute(channel, client) {
        ...
        if (antiraid?.actived) {
            ...
            if (!executor) return;
            if (executor.id === "928411318589988884") {
                return;
            }
            ...
            if (user && guild.me.roles.highest.comparePositionTo(user?.roles.highest) <= 0) {
                return;
            }
            if (!userlogs) {
                ...
                if (userlogs?.ChannelsCreate + 1 === antiraid?.maxChannelsCreate) {
                    if (antiraid?.sancionType === "ban") {
                        ...
                        guild.members.kick(user, `${emojis.positivo} | Anti-Raid esta activado`).catch(e => { });
                        if (raidlogs?.actived) {
                            const canal = await guild.channels.cache.get(raidlogs?.channelId);
                            if (canal) {
                                canal.send({
